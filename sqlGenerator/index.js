...
function exhibition(number) {
	...
	for (let i = 0; i < number; i++) {
      ...
      VALUES ('${i + 1}', '${faker.internet.userName()}', '${faker.lorem.sentence()}', '${faker.lorem.sentence()}', '${
...
function artwork(number) {
    ...
    VALUES ('${i + 1}', '${faker.lorem.word()}', '${types[randomInt(types.length)]}', '${
			randomInt(100) / 100
		}', '${faker.lorem.sentence()}', 'NoBid', '${faker.lorem.sentence()}', ${
			isExhibitionSelect[i] ? `'${isExhibitionSelect[i]}'` : "null"
		},'${userIndex}', '${userIndex}', '${images[selectImage]}', '${cropped_image[selectImage]}');`);
	}
	return result.join("\n");
}
...
function acution(number) {
	...
	for (let i = 0; i < number; i++) {
        ...
        VALUES ('${i + 1}', CURRENT_DATE, CURRENT_DATE + ${randomInt(10) + 1}, '${userIndex}', '${artworkIndex}');
        `);
		result.push(
